...
/*Atualização Master Hub (Campo Complemento no Catálogo de Endereços)*/
	    ...
	    $(".telefone_contato").attr("placeholder",  getTrans("Telefone de Contato",'telefone_contato_deste') );
/*Fim da atualização*/
...
/*Fim da atualização*/
		...
		case "page-shipping-location-area-coleta": 
			
			translatePage();
		   $("#frm-coleta .street").attr("placeholder", getTrans('Street','street') );
		   $("#frm-coleta .contact_phone").attr("placeholder", getTrans('Contact phone','contact_phone') );
	       $("#frm-coleta .location_name").attr("placeholder", getTrans('Apartment suite, unit number, or company name','location_name2') );
	       $("#frm-coleta .delivery_instruction").attr("placeholder", getTrans('Delivery instructions','delivery_instruction') );
		   
		   if ( !empty(global_city_id)){
		   	   $("#frm-coleta .location_city").html( global_city_name );
		   	   
		   	   $("#frm-coleta .city").val( global_city_name );
		   	   $("#frm-coleta .city_id").val( global_city_id );
		   }
		   
		   if ( !empty(global_area_id)){
		   	   $("#frm-coleta .location_area").html( global_area_name );
		   	   
		   	   $("#frm-coleta .state").val( global_area_name );
		   	   $("#frm-coleta .area_id").val( global_area_id );
		   }
/*Atualização Master Hub (Catálogo de Endereços na finalização do Pedido)*/
		   	switch (search_type)
			{
				...
				case 1:
				//$(".location_state").hide();
				$("#frm-coleta .location_postal").hide();
		   if ( !empty(global_state_id)){
		   	   $("#frm-coleta .location_state").html( global_state_name );		   	   		   	   
		   	   $("#frm-coleta .state_id").val( global_state_id );
		   	   $("#frm-coleta .state").val( global_state_name );
				}
				if ( !empty(global_city_id)){
					$("#frm-coleta .city_id").val( global_city_id );
					$("#frm-coleta .city").val( global_city_name );
				...
				if ( !empty(global_area_id)){
					$("#frm-coleta .area_id").val( global_area_id );
					$("#frm-coleta .area_name").val( global_area_name );
					$("#frm-coleta .location_area").html( global_area_name );
		   }
				break;
		   ...
		   if ( !empty(global_state_id)){
				...
				$("#frm-coleta .location_state").show();
				$("#frm-coleta .location_area").show();
				$("#frm-coleta .location_postal").hide();
				break;

				case "3":
				case 3:
				$("#frm-coleta .location_state").hide();
				$("#frm-coleta .location_city").hide();
				$("#frm-coleta .location_area").hide();
				$("#frm-coleta .location_postal").show();
				break;
			}
/*Fim da atualização*/
           translateValidationForm();
/*Atualização Master Hub (Desativa entrada do código do país nos campos de telefone)*/
           //*initIntelInputs();
/*Fim da atualização*/
			var customer_contact_number;
			if (!empty(getStorage("customer_contact_number"))){
            customer_contact_number=getStorage("customer_contact_number");
			}
			
	       if(!empty(customer_contact_number)){
			  $("#frm-coleta .telefone_contato_titulo").show();  
			  $("#frm-coleta .contact_phone_titulo").hide(); 
	      	  $("#frm-coleta .contact_phone").val(customer_contact_number.replace("+55",""));
	       } else {
			  $("#frm-coleta .contact_phone_titulo").show();  
			  $("#frm-coleta .telefone_contato_titulo").hide();   
		   }
		...
		case "page-shipping-location-area": 
			...
			translatePage();
		   $("#frm-shipping .street").attr("placeholder", getTrans('Street','street') );
		   $("#frm-shipping .contact_phone").attr("placeholder", getTrans('Contact phone','contact_phone') );
	       $("#frm-shipping .location_name").attr("placeholder", getTrans('Apartment suite, unit number, or company name','location_name2') );
	       $("#frm-shipping .delivery_instruction").attr("placeholder", getTrans('Delivery instructions','delivery_instruction') );
		   
		   if ( !empty(global_city_id)){
		   	   $("#frm-shipping .location_city").html( global_city_name );
		   	   
		   	   $("#frm-shipping .city").val( global_city_name );
		   	   $("#frm-shipping .city_id").val( global_city_id );
		   }
		   
		   if ( !empty(global_area_id)){
		   	   $("#frm-shipping .location_area").html( global_area_name );
		   	   
		   	   $("#frm-shipping .state").val( global_area_name );
		   	   $("#frm-shipping .area_id").val( global_area_id );
		   }
/*Atualização Master Hub (Catálogo de Endereços na finalização do Pedido)*/
		   	switch (search_type)
			{
				...
				case 1:
				//$(".location_state").hide();
				$("#frm-shipping .location_postal").hide();
		   if ( !empty(global_state_id)){
		   	   $("#frm-shipping .location_state").html( global_state_name );		   	   		   	   
		   	   $("#frm-shipping .state_id").val( global_state_id );
		   	   $("#frm-shipping .state").val( global_state_name );
				}
				if ( !empty(global_city_id)){
					$("#frm-shipping .city_id").val( global_city_id );
					$("#frm-shipping .city").val( global_city_name );
				...
				if ( !empty(global_area_id)){
					$("#frm-shipping .area_id").val( global_area_id );
					$("#frm-shipping .area_name").val( global_area_name );
					$("#frm-shipping .location_area").html( global_area_name );
		   }
				break;
		   ...
		   if ( !empty(global_state_id)){
				...
				$("#frm-shipping .location_state").show();
				$("#frm-shipping .location_area").show();
				$("#frm-shipping .location_postal").hide();
				break;

				case "3":
				case 3:
				$("#frm-shipping .location_state").hide();
				$("#frm-shipping .location_city").hide();
				$("#frm-shipping .location_area").hide();
				$("#frm-shipping .location_postal").show();
				break;
			}
/*Fim da atualização*/
           translateValidationForm();
...
/*Fim da atualização*/
	       ...
	       if(!empty(customer_contact_number)){
			  $("#frm-shipping .telefone_contato_titulo").show();  
			  $("#frm-shipping .contact_phone_titulo").hide(); 
	      	  $("#frm-shipping .contact_phone").val(customer_contact_number.replace("+55",""));
	       } else {
			  $("#frm-shipping .contact_phone_titulo").show();  
			  $("#frm-shipping .telefone_contato_titulo").hide();   
		   }
	       
		   break;
		
		case "page-shipping-location-area-pre-coleta":   
		
		   translatePage();
		   $("#frm-pre-coleta .street").attr("placeholder", getTrans('Street','street') );
		   $("#frm-pre-coleta .contact_phone").attr("placeholder", getTrans('Contact phone','contact_phone') );
	       $("#frm-pre-coleta .location_name").attr("placeholder", getTrans('Apartment suite, unit number, or company name','location_name2') );
	       $("#frm-pre-coleta .delivery_instruction").attr("placeholder", getTrans('Delivery instructions','delivery_instruction') );
		   
		   if ( !empty(global_city_id)){
		   	   $("#frm-pre-coleta .location_city").html( global_city_name );
		   	   
		   	   $("#frm-pre-coleta .city").val( global_city_name );
		   	   $("#frm-pre-coleta .city_id").val( global_city_id );
		   }
		   
		   if ( !empty(global_area_id)){
		   	   $("#frm-pre-coleta .location_area").html( global_area_name );
		   	   
		   	   $("#frm-pre-coleta .state").val( global_area_name );
		   	   $("#frm-pre-coleta .area_id").val( global_area_id );
		   }
/*Atualização Master Hub (Catálogo de Endereços na finalização do Pedido)*/
		   	switch (search_type)
			{
				...
				case 1:
				//$(".location_state").hide();
				$("#frm-pre-coleta .location_postal").hide();
		   if ( !empty(global_state_id)){
		   	   $("#frm-pre-coleta .location_state").html( global_state_name );		   	   		   	   
		   	   $("#frm-pre-coleta .state_id").val( global_state_id );
		   	   $("#frm-pre-coleta .state").val( global_state_name );
				}
				if ( !empty(global_city_id)){
					$("#frm-pre-coleta .city_id").val( global_city_id );
					$("#frm-pre-coleta .city").val( global_city_name );
				...
				if ( !empty(global_area_id)){
					$("#frm-pre-coleta .area_id").val( global_area_id );
					$("#frm-pre-coleta .area_name").val( global_area_name );
					$("#frm-pre-coleta .location_area").html( global_area_name );
		   }
				break;
		   ...
		   if ( !empty(global_state_id)){
				...
				$("#frm-pre-coleta .location_state").show();
				$("#frm-pre-coleta .location_area").show();
				$("#frm-pre-coleta .location_postal").hide();
				break;

				case "3":
				case 3:
				$("#frm-pre-coleta .location_state").hide();
				$("#frm-pre-coleta .location_city").hide();
				$("#frm-pre-coleta .location_area").hide();
				$("#frm-pre-coleta .location_postal").show();
				break;
			}
/*Fim da atualização*/
           translateValidationForm();
...
/*Fim da atualização*/
	       ...
	       if(!empty(customer_contact_number)){
			  $("#frm-pre-coleta .telefone_contato_titulo").show();  
			  $("#frm-pre-coleta .contact_phone_titulo").hide(); 
	      	  $("#frm-pre-coleta .contact_phone").val(customer_contact_number.replace("+55",""));
	       } else {
			  $("#frm-pre-coleta .contact_phone_titulo").show();  
			  $("#frm-pre-coleta .telefone_contato_titulo").hide();   
		   }
		 ...
		 case "page-cart":
		   ...
		   if (typeof search_address === "undefined" || search_address==null || search_address=="" || search_address=="null" ) { 	    		   			  	  
			...
			botao_cancelar(getStorage("tipo_de_servico"));
		  ...
		  var cart_params=JSON.stringify(cart);       	        	  
      	  ...
      	  if(empty(getStorage("transaction_type"))){      	  	      	  	
      	  	switch (getStorage("merchant_services"))
      	  	{
      	  		...
      	  		case "10":
      	  		case "11":
					var modo_servico=explode("|",cart_params); 
      	  		params+="&transaction_type="+modo_servico[1];
      	  		setStorage('transaction_type',modo_servico[1]);
      	  		break;
					
      	  		case 12:
      	  		case "12":
      	  		params+="&transaction_type=prestacao_servico";
...
function applySearch() {
	...
	sparams2 = "address="+ getStorage("search_address") + "&restaurant_name="+ ss_restaurant_name;

	search_mode2 = getSearchMode();

	if ( search_mode2=="postcode"){

		sparams2+="&search_mode="+ search_mode2;
		search_type = getSearchType();
		switch (search_type) {

			case "1":				
			sparams2+="&city_id="+ global_city_id;
			sparams2+="&area_id="+ global_area_id;
			break;
			
			case "2":
			sparams2+="&state_id="+ global_state_id;
			sparams2+="&city_id="+ global_city_id;
			break;
			
			case "3":
			sparams2+="&postal_code="+ global_postal_code;
			break;
...
if (data.details.default_address.area_id==0 || data.details.default_address.city_id==0){
		...
		customer_contact_number  = data.details.contact_phone;
...
/*Fim da atualização*/
				...
				dump(data.details.tempo_de_entrega);
				dump(data.details.tipo_de_servico);	
				setStorage("tipo_de_servico", data.details.tipo_de_servico);
...
/* Fim da Atualização */
				...
				if ( data.details.transaction_type=="delivery" || data.details.transaction_type=="coleta" || data.details.transaction_type=="coleta_retorno" || data.details.transaction_type=="pre_coleta" || data.details.transaction_type=="pre_coleta_retorno" || data.details.transaction_type=="prestacao_servico"){									   
				...
				if ( data.details.delivery_options){
					...
					data_da_entrega=getStorage("data_da_entrega");
					hora_da_entrega=getStorage("hora_da_entrega");
					dump(hora_da_entrega);
					dump(data_da_entrega);
					if (data_da_entrega==null || data_da_entrega=='null'){
					...
					$(".delivery_date").val( data_da_entrega );
					$(".cart_delivery_date_value").html( data.details.delivery_options.default_value_date.date_pretty );
					}
					
					if (hora_da_entrega==null || hora_da_entrega=='null'){
					...
					$(".delivery_time").val( hora_da_entrega );
					$(".cart_delivery_time_value").html( data.details.delivery_options.default_value_time.time_pretty );
					}
				}
...
/*Fim da atualização*/
				    ...
				    } else if ( data.details=="coleta"){		
				    	
				    	search_mode = getSearchMode();
				    	if (search_mode=="postcode"){

				    		showShippingLocationColeta(data);			    		
...
/*Atualização Master Hub (Correção de tela errada de endereços finalizando o pedido)*/
						    sNavigator.pushPage("shippingLocationAreaColeta.html", options);
/*Fim da atualização*/
						    
				    	}				
					    		
				    } else if ( data.details=="coleta_retorno"){		
...
/*Fim da atualização*/
				    ...
				    } else if ( data.details=="pre_coleta"){		
				    	
				    	search_mode = getSearchMode();
				    	if (search_mode=="postcode"){

				    		showShippingLocationPreColeta(data);			    		
...
/*Atualização Master Hub (Correção de tela errada de endereços finalizando o pedido)*/
						    sNavigator.pushPage("shippingLocationAreaPreColeta.html", options);
/*Fim da atualização*/
						    
				    	}				
					    		
				    } else if ( data.details=="pre_coleta_retorno"){		
...
/*Fim da atualização*/
					...
					} else if (data.details.next_step=="coleta_address"){
...
/*Fim da atualização*/
					...
					} else if (data.details.next_step=="pre_coleta_address"){
...
/*Atualização Master Hub (Resumo de Endereço na finalização do pedido)*/
		transaction_type=getStorage("transaction_type");
					if (transaction_type=="delivery" || transaction_type=="prestacao_servico"){
					$(".conferirendereco").show();	
					$(".conferirenderecoentrega").hide();
					$(".conferirenderecocoleta").hide();
					$(".conferirenderecoprecoleta").hide();	
					$(".conferirenderecoretorno").hide();	
					}else if (transaction_type=="coleta"){
					...
					} else if (transaction_type=="coleta_retorno"){
					$(".conferirendereco").hide();	
					$(".conferirenderecocoleta").show();
					$(".conferirenderecoentrega").show();
					$(".conferirenderecoprecoleta").hide();	
					...
					} else if (transaction_type=="pre_coleta"){
					$(".conferirendereco").hide();	
					$(".conferirenderecoprecoleta").show();	
					$(".conferirenderecocoleta").show();
					$(".conferirenderecoentrega").show();
					$(".conferirenderecoretorno").hide();	
						setStorage('merchant_address',getStorage("coleta_address"));
					} else if (transaction_type=="pre_coleta_retorno"){
					$(".conferirendereco").hide();	
					$(".conferirenderecoprecoleta").show();	
					$(".conferirenderecocoleta").show();
					$(".conferirenderecoentrega").show();
	     ...
	     $(".total-entrega").html(prettyPrice(entrega));
					...
					if (getStorage("precoleta_address")){
					$(".endereco_de_precoleta").html(getStorage("precoleta_address"));	
					}
			      ...
			      $(".receipt-msg").html(data.msg); 
                              ...
                              $htmendentrega='';
                               $enderecoico ="";
                                $enderecodesc="";
                              dump(data);
                              if(getStorage("transaction_type")=='delivery' || getStorage("transaction_type")=="coleta" || getStorage("transaction_type")=="coleta_retorno" || getStorage("transaction_type")=="pre_coleta" || getStorage("transaction_type")=="pre_coleta_retorno" || getStorage("transaction_type")=="prestacao_servico"){ //se for entrega
...
/*Fim da atualização*/
                   ...
                   } else if ( data.details.is_checkout=="coleta_address"){
...
/*Fim da atualização*/
						    ...
						    sNavigator.pushPage("shippingColeta.html", options);	
                   	    }
					    
                   } else if ( data.details.is_checkout=="pre_coleta_address"){
...
/*Fim da atualização*/
						    ...
						    sNavigator.pushPage("shippingPreColeta.html", options);	
                   	    }
			    ...
			    case "getLocationCity":
			      ...
			      var htm='';
			       ...
			       $.each( data.details, function( city_key , city_val ) { 
			       	 ...
			       	 htm+='<ons-list-item modifier="tappable" onclick="setLocationCity('+"'"+city_val.id+"',"+"'"+city_val.name+"',"+  "'" + city_val.state_id +"',"+"'"+city_val.estado+"'" +  ');">';
					 htm+='<label class="radio-button checkbox--list-item">';
						htm+='<input type="radio" name="channel_m" class="channel_m" value="'+city_val.id+'"  >';
						htm+='<div class="radio-button__checkmark checkbox--list-item__checkmark"></div>';
						htm+=' '+city_val.name+'/'+city_val.estado;
					  htm+='</label>'; 
					htm+='</ons-list-item>';
...
/* Fim da Atualização */
			    ...
			    case "getReceipt":
			      ...
			      $.each( data.details.info, function( key, val ) {     
			      	 dump(val);
			      	 html+=wingRow( '', val.label, val.value, val.phone );
			      });
			      html+='</ons-list>';
...
/*Fim da atualização*/
    $.each( data, function( key, val ) {     
		...
		if (val.is_open=='Fechado'){
	    	           ...
	    	           //if(val.service!=3){
	    	           if(val.service==1 || val.service==2 || val.service==4 || val.service==5 || val.service==8 || val.service==9 || val.service==10 || val.service==11 || val.service==12){
	    	           ...
	    	           if(val.service==8 || val.service==9 || val.service==10 || val.service==11 || val.service==12){
	    	             htm+='<ons-icon class="stic-icon" icon="ion-android-bicycle"></ons-icon>';
	    	             if(!empty(val.delivery_fee)){
	    	                htm+='<price>'+val.delivery_fee+'</price>';
	    	             }
         ...
         if ( data.mobile_menu==1){
             ...
             html+='<ons-col class="col-image text-right" width="35%">';
              if ( val.prices.length>0){
	                $.each( val.prices, function( key_price, price ) { 
	                   if (!empty(price.price_discount_pretty)){
	                   	   html+='<p class="p-small">'+  getTrans(price.size, price.size)  +' <price class="discount">'+price.price_pretty+'</price>'; 
	                   	   html+='<price>'+price.price_discount_pretty+'</price>';
	                   	   html+='</p>';
	                   } else {
	                   	   html+='<p class="p-small">'+  getTrans(price.size, price.size)  +' <price>'+price.price_pretty+'</price></p>';
	                   }                   
	                });
                }
             html+='</ons-col>';
...
/*Atualização Master Hub (Mostra logo da empresa quando não há logo do produto.)*/
		...
		dump("upload_url=>"+upload_url); 
             ...
             html+='</ons-col>';
                ...
                if ( val.prices.length>0){
					if (data.tipo_de_servico=='8' || data.tipo_de_servico=='9' || data.tipo_de_servico=='10' || data.tipo_de_servico=='11'){
	                $.each( val.prices, function( key_price, price ) { 
	                   if (!empty(price.price_discount_pretty)){
	                   	   html+='<p class="p-small stic-price">'+  getTrans(price.size, price.size); 

	                   	   html+='</p>';
	                   } else {
	                   	   html+='<p class="p-small">'+  getTrans(price.size, price.size)+'</p>';
	                   }                   
	                });
		} else {
	                $.each( val.prices, function( key_price, price ) { 
	                   if (!empty(price.price_discount_pretty)){
	                   	   html+='<p class="p-small stic-price">'+  getTrans(price.size, price.size)  +' <price class="discount stic-price">'+price.price_pretty+'</price>'; 
	                   	   html+='<price>'+price.price_discount_pretty+'</price>';
	                   	   html+='</p>';
	                   } else {
	                   	   html+='<p class="p-small">'+  getTrans(price.size, price.size)  +' <price>'+price.price_pretty+'</price></p>';
                ...
                if (val.not_available==2){
                	...
                	html+='<p class="stic-notavailable" >'+  getTrans("item not available",'item_not_available')  +'</p>';
/*Fim da atualização*/
                }
...
{	
	...
	htm+='<input type="hidden" name="dois_sabores" class="dois_sabores" value="'+data.dois_sabores+'">';	
	...
	} else if ( data.has_price==3){	
		htm+='<ons-list-header class="list-header trn" data-trn-key="escolha_uma_opcao">Escolha uma opcao</ons-list-header>';
		var x=0
		$.each( data.prices, function( key, val ) { 				
			if (data.discount>0){
				var discount_price='<price class="discount">'+val.pretty_price;				
...
/*Fim da atualização*/
	...
	if (data.tipo_de_servico=="8" || data.tipo_de_servico=="9" || data.tipo_de_servico=="10" || data.tipo_de_servico=="11" || data.tipo_de_servico=="12"){
	htm+=cartFooter_coleta(data.currency_code);
	} else {
...
$( document ).on( "click", ".prod-qtd-menos", function(){
   ...
   // Don't allow decrementing below zero
	...
	$( document ).on( "click", ".sub_item_sabor", function() {		
		 var this_obj=$(this);
		 var multi=$(this).data("multi");		 
		 if (empty(multi)){
		 	return;
		 ...
		 var id=$(this).data("id");
		 var sabores=$(this).data("sabores");
		 var total_check=0;		 	
		 $('.sub_item_sabor:checked').each(function(){ 
		 	if ( $(this).data("sabores") == sabores){
		 		total_check++;
		 	}
		 });		 
		 if (multi<total_check){
...
/*Fim da atualização*/
		...
		switch( $(this).val() )
		{
			...
			if ( transaction_type =="delivery" || transaction_type =="pickup" || transaction_type =="dinein" || transaction_type=="coleta" || transaction_type=="coleta_retorno" || transaction_type=="pre_coleta" || transaction_type=="pre_coleta_retorno") {
...
/*Atualização Master Hub (Catálogo de Endereços e Correção de preenchimento na finalização)*/
			...
			$(".telefone_contato").val( address_split[12] );
			
			var number='';
			if (!empty(address_split[7])){
				if (!empty(address_split[12])){
				$(".telefone_contato_titulo").show();	
				$(".contact_phone_titulo").hide();
				} else {
				//$(".contact_phone").show();	
				}
				number=getStorage("customer_contact_number");	
			}
...
{	
	...
	$('#page-itemdisplay .sub_item:checkbox:checked').each(function(){        		
        ...
        } else if ( $(this).data("sabores")=="limitado" ){ 
		dump(addo_price);
        dump(addo_price[1]);
...
{		
	...
	if ( $(".require_addon").exists()){
		...
		$('.require_addon').each(function () {
			if ( $(this).val()==2 ) {
				var required_addon_id=$(this).data("id");
	   	   	   	var required_addon_name=$(this).data("name");
	if ($(this).data("multi")=='' || $(this).data("multi")=='undefined' || $(this).data("multi")=='null'){
		var required_addon_multi=0;
		} else {
			var required_addon_multi=$(this).data("multi");
		}
		var required_addon_selected=$(".sub_item_name_"+required_addon_id+":checked").length; 
	if ( required_addon_selected <= required_addon_multi-1){
	   	   	   		proceed=false;
	   	   	   			   	   	  
	var contagem_selected2=required_addon_multi-required_addon_selected;	   	  
	var err_msg= getTrans("Voce ainda precisa selecionar","voce_ainda_precisa_selecionar")+" ("+ contagem_selected2+") "+getTrans("itens na secao","itens_em") +" - "+ required_addon_name;
	...
	/*check if sub item has required*/
	if ( $(".require_sabores").exists()){
		$(".small-red-text").remove();	
		$('.require_sabores').each(function () {
			if ( $(this).val()==2 ) {
				//var required_sabores_id=$(this).data("id");
				var sabores=$(this).data("sabores");
				
				if ($(this).data("multi")=='' || $(this).data("multi")=='undefined' || $(this).data("multi")=='null'){
					var required_sabores_multi=0;
				} else {
					var required_sabores_multi=$(this).data("multi");
				}
	   	   	   	var required_sabores_name="Sabores do produto";
	   	   	   	var required_sabores_selected=$(".sub_item_sabor:checked").length; 
	   	   	   	if ( required_sabores_selected <= required_sabores_multi-1){
	   	   	   		proceed2=false;
	   	   	   			 var contagem_selected=required_sabores_multi-required_sabores_selected;	   	  
	   	   	   		var err_msg= getTrans("Voce ainda precisa selecionar","voce_ainda_precisa_selecionar")+" ("+ contagem_selected+") "+getTrans("itens na secao","itens_em") +" - "+ required_sabores_name;
	...
	var order_notes='';
	var discount='';
	var tipo_de_servico = getStorage("tipo_de_servico");
	...
	if (!empty(params)){
		...
		if ( $(".dois_sabores").val()==2 ){			
			var sub_item_selected=$(".sub_item:checked").length; 			
			 ...
			 if ( $(this).data("withqty")==2 ){
				...
				//var valor_sabores= sabor_price[0]+'|'+0+'|'+sabor_price[2];
			    var valor_sabores= sabor_price[0]+'|'+0+'|'+$(this).data("nome_produto");
				var addon_total=0;
        ...
        dump("selected_price=>"+selected_price);
		price=qty* parseFloat(selected_price);
		...
		//addon_total = Math.max.apply(Math, sabor_price_array);
			...
			if(two_flavor_option==2){
				if($.isArray(sabor_price_array)) {
					var dois_sabores_sum = 0;
					$.each( sabor_price_array , function( key_1, val_1 ) {							
						dois_sabores_sum+= parseFloat(val_1);
					});
					
					dump("dois_sabores_sum =>" +dois_sabores_sum);
					if(dois_sabores_sum>0){
					  addon_total = dois_sabores_sum/multi;
					}
				}
			} else {
			   addon_total = Math.max.apply(Math, sabor_price_array);
			...
			dump("addon_total price => "+addon_total);
			price += parseFloat(addon_total);
...
/*Fim da atualização*/
			 ...
			 if (!empty(val.sub_item)){
			 	...
			 	$.each( val.sub_item , function( key_sub, val_sub ) {			 		 
				 	 ...
				 	 $.each( val_sub  , function( key_sub2, val_sub2 ) {			 		 
						 ...
						 if ( val_sub2.total_pretty ==0 || val_sub2.total_pretty =='0'){
				 	      	 subitem_total="-";
				 	      } else {
				 	      	 subitem_total=val_sub2.total_pretty;
				 	      }	
				 	      ...
				 	      htm+=tplCartRowNoBorderSub(
				 	        ...
				 	        //val_sub2.price,
						val_sub2.price>0?val_sub2.price:'-',
	...
	setStorage('transaction_type',transaction_type);
		...
		if (!empty(data.cart.sub_total)){
		if (transaction_type=='prestacao_servico'){
			htm+=tplCartRow( getTrans('Sub Total','sub_total') , data.cart.sub_total.amount_pretty ,'price-normal');
			} else if (transaction_type=='coleta' || transaction_type=='pre_coleta' || transaction_type=="coleta_retorno" || transaction_type=='pre_coleta_retorno'){
			htm+=tplCartRow( getTrans('Valor do Servico','valor_servico') , data.cart.sub_total.amount_pretty ,'price-normal');
			} else {
			htm+=tplCartRow( getTrans('Sub Total','sub_total') , data.cart.sub_total.amount_pretty ,'price-normal');
			}
		}
		...
		if (!empty(data.cart.delivery_charges)){
		if (transaction_type=='prestacao_servico'){
			htm+=tplCartRow( getTrans('Custo Deslocamento','custo_deslocamento') , data.cart.delivery_charges.amount_pretty, 'price-normal');
			} else if (transaction_type=='coleta' || transaction_type=='pre_coleta'){
			htm+=tplCartRow( getTrans('Custo de Coleta e Entrega','coleta_e_entrega') , data.cart.delivery_charges.amount_pretty, 'price-normal');
			} else if (transaction_type=='coleta_retorno' || transaction_type=='pre_coleta_retorno'){
			htm+=tplCartRow( getTrans('Custo de Entrega e Retorno','entrega_e_retorno') , data.cart.delivery_charges.amount_pretty, 'price-normal');
			} else {
	...
	switch (data.merchant_info.service)
	{
		...
		case 8:
		case "8":		    
			htm+=privateRowWithRadio('transaction_type',data.cart.cart[0].size, getTrans('Coleta & Entrega',data.cart.cart[0].size) );
		break;
		...
		case 12:
		case "12":		    
			htm+=privateRowWithRadio('transaction_type','prestacao_servico', getTrans('Fazer Servico no local','fazer_no_local') );
			htm+=privateRowWithRadio('transaction_type','delivery', getTrans('Nos Buscamos o Equipamento','buscar_equipamento') );	    
		    htm+=privateRowWithRadio('transaction_type','pickup',  getTrans('Levar pessoalmente','levar_pessoalmente') );
		break;
...
function editOrderInit()
{
	...
	$(".row-del-wrap").css("margin-top","-44px");
	$(".width-adjust").css("max-width","70%");
	$(".effeckt-button").css("height","32px");
	$(".effeckt-button").css("background","brown");
...
/* início */	
	...
	$.validate({ 	
	    ...
	    onSuccess : function() {  
	       var params2 = $( "#frm-coleta").serialize();	
		      	  var params = $("#frm-coleta .location_name").val();
		      	  params+=" | "+ $("#frm-coleta .street").val();
		      	  params+=" nº "+ $("#frm-coleta .numero").val();
		      	  params+=" - "+ $("#frm-coleta .delivery_instruction").val();
		      	  params+=" - "+ $("#frm-coleta .area_name").val();
		      	  params+=" - "+ $("#frm-coleta .city").val();
		      	  params+="/"+ $("#frm-coleta .state").val();
	...
	if ( !empty($(".delivery_time").val()) ){
		extra_params+="&delivery_time="+$(".delivery_time").val();
			...
			extra_params+="&telefone_contato="+ urlencode($("#frm-coleta .telefone_contato").val());
			extra_params+="&street="+ urlencode($("#frm-coleta .street").val());
			extra_params+="&numero="+ urlencode($("#frm-coleta .numero").val());
			extra_params+="&area_name="+ urlencode($("#frm-coleta .area_name").val());
			extra_params+="&city="+ urlencode($("#frm-coleta .city").val());
			extra_params+="&state="+ urlencode($("#frm-coleta .state").val());
			extra_params+="&zipcode="+ urlencode($("#frm-coleta .zipcode").val());
			extra_params+="&state_id="+ urlencode($("#frm-coleta .state_id").val());
			extra_params+="&city_id="+ urlencode($("#frm-coleta .city_id").val());
			extra_params+="&area_id="+ urlencode($("#frm-coleta .area_id").val());
			extra_params+="&delivery_instruction="+ urlencode($("#frm-coleta .delivery_instruction").val());
			extra_params+="&location_name="+ urlencode($("#frm-coleta .location_name").val());
			extra_params+="&save_address="+$('#frm-coleta .save_address:checked').val();
	...
	setTrackView("checkoutcoleta");
	
    callAjax("checkoutcoleta","merchant_id="+ getStorage('merchant_id')+"&search_address=" + 
      	  encodeURIComponent(getStorage("search_address")) + "&transaction_type=" + 
      	  getStorage("transaction_type") + extra_params );	
...
/* início */	
	...
	$.validate({ 	
	    ...
	    onSuccess : function() {  
	       var params2 = $( "#frm-pre-coleta").serialize();	
		      	  var params = $("#frm-pre-coleta .location_name").val();
		      	  params+=" | "+ $("#frm-pre-coleta .street").val();
		      	  params+=" nº "+ $("#frm-pre-coleta .numero").val();
		      	  params+=" - "+ $("#frm-pre-coleta .delivery_instruction").val();
		      	  params+=" - "+ $("#frm-pre-coleta .area_name").val();
		      	  params+=" - "+ $("#frm-pre-coleta .city").val();
		      	  params+="/"+ $("#frm-pre-coleta .state").val();
		      	  params+=" - "+ $("#frm-pre-coleta .zipcode").val();
			
					setStorage('precoleta_address',params);
			        setStorage('precoleta_address_phone', $("#frm-pre-coleta .telefone_contato").val());
	...
	if ( $(".required_time").val()==2){
		if ( $(".delivery_time").val() ==""){			
			...
			extra_params+="&telefone_contato="+ urlencode($("#frm-pre-coleta .telefone_contato").val());
			extra_params+="&street="+ urlencode($("#frm-pre-coleta .street").val());
			extra_params+="&numero="+ urlencode($("#frm-pre-coleta .numero").val());
			extra_params+="&area_name="+ urlencode($("#frm-pre-coleta .area_name").val());
			extra_params+="&city="+ urlencode($("#frm-pre-coleta .city").val());
			extra_params+="&state="+ urlencode($("#frm-pre-coleta .state").val());
			extra_params+="&zipcode="+ urlencode($("#frm-pre-coleta .zipcode").val());
			extra_params+="&state_id="+ urlencode($("#frm-pre-coleta .state_id").val());
			extra_params+="&city_id="+ urlencode($("#frm-pre-coleta .city_id").val());
			extra_params+="&area_id="+ urlencode($("#frm-pre-coleta .area_id").val());
			extra_params+="&delivery_instruction="+ urlencode($("#frm-pre-coleta .delivery_instruction").val());
			extra_params+="&location_name="+ urlencode($("#frm-pre-coleta .location_name").val());
			extra_params+="&save_address="+$('#frm-pre-coleta .save_address:checked').val();
	...
	setTrackView("checkoutprecoleta");
	
    callAjax("checkoutprecoleta","merchant_id="+ getStorage('merchant_id')+"&search_address=" + 
      	  encodeURIComponent(getStorage("search_address")) + "&transaction_type=" + 
      	  getStorage("transaction_type") + extra_params );	
	     ...
	     // setStorage("customer_contact_number", $(".contact_phone").val().replace(/[^0-9\.]+/g, ''));
	      ...
	      var params = "";
				...
				jQuery.each( form, function(i, field) {
					...
					setStorage('customer_contact_number',field.value);
				});
/*Fim da atualização*/
	      params+="&transaction_type=" +  getStorage("transaction_type") ;
	      params+="&device_id="+ getStorage("device_id");
...
{	
	...
	$.validate({ 	
	    ...
	    onSuccess : function() {  
		   ...
		   setStorage('shipping_address_phone', $("#frm-shipping .telefone_contato").val());
	       dump(params);	       
	       var options = {
		      animation: 'slide',
		      onTransitionEnd: function() { 						      	  
...
/*Fim da atualização*/
		      	  ...
		      	  params+="&merchant_id="+ getStorage("merchant_id");
		      	  params+="&street="+ urlencode($("#frm-shipping .street").val());
/*Atualização Master Hub (Catálogo de Endereços e Número e Bairro)*/
		      	  params+="&numero="+ urlencode($("#frm-shipping .numero").val());
		      	  params+="&area_name="+ urlencode($("#frm-shipping .area_name").val());
		      	  params+="&city="+ urlencode($("#frm-shipping .city").val());
		      	  params+="&state="+ urlencode($("#frm-shipping .state").val());
		      	  params+="&zipcode="+ urlencode($("#frm-shipping .zipcode").val());
			  params+="&state_id="+ urlencode($("#frm-shipping .state_id").val());
			  params+="&city_id="+ urlencode($("#frm-shipping .city_id").val());
			  params+="&area_id="+ urlencode($("#frm-shipping .area_id").val());
			  params+="&delivery_instruction="+ urlencode($("#frm-shipping .delivery_instruction").val());
/*Fim da atualização*/
		      	  params+="&location_name="+ urlencode($("#frm-shipping .location_name").val());
		      	  params+="&save_address="+$('#frm-shipping .save_address:checked').val();
		      	  params+="&transaction_type=" +  getStorage("transaction_type") ;
		      	  params+="&client_token="+ getStorage('client_token');
		      	  params+="&contact_phone="+ $("#frm-shipping .contact_phone").val();
		      	  params+="&telefone_contato="+ $("#frm-shipping .telefone_contato").val();
...
if (getStorage("transaction_type")=="pre_coleta_retorno"){
		...
		extra_params+="&delivery_asap="+ $(".delivery_asap:checked").val();		
		extra_params+="&mobile_app=mobile_app_android";		
		...
		if (!empty(getStorage('coleta_address'))){
		extra_params+="&coleta_address="+ getStorage('coleta_address');
		extra_params+="&coleta_address_phone="+ getStorage('coleta_address_phone');	
		}
		if (!empty(getStorage('precoleta_address'))){
		extra_params+="&pre_coleta_address="+ getStorage('precoleta_address');	
		extra_params+="&pre_coleta_address_phone="+ getStorage('precoleta_address_phone');	
		}
		if (!empty(getStorage('retorno_address'))){
		extra_params+="&retorno_address="+ getStorage('retorno_address');	
		}
		...
		extra_params+="&shipping_address_phone="+ getStorage('shipping_address_phone');	
		extra_params+="&client_token="+ getStorage('client_token');
	...
	if ( data.address.length>0){		
	   $.each( data.address, function( key, val ) {  
	     ...
	     htm+='<ons-list-item modifier="tappable" onclick="modifyAddressBook('+val.id+');" >';
		        htm+='<p class="small-font-dim" style="font-size: 17px; color: #960000; margin: 5px 0px -7px 0px;">'+val.location_name+'</p>';
	         htm+='<ons-row class="row">';
			    htm+='<ons-col class="" width="90%">';
	...
	if ( data.address.length>0){		
	   $.each( data.address, function( key, val ) {   		
		 ...
		 complete_address+=val.state_id+"|";
	   	 ...
	   	 complete_address+=val.telefone_contato+"|";
...
function reloadCart()
{
	...
	params+="&device_id="+ getStorage("device_id");
	params+="&tips_percentage=" + getStorage("tips_percentage");
	params+="&delivery_time=" + getStorage("hora_da_entrega");
	params+="&delivery_date=" + getStorage("data_da_entrega");
...
function setLocationCity(city_id, city_name, state_id, state_name)
{
...
{  
  ...
  removeStorage('coleta_address');  
  removeStorage('precoleta_address');  
  removeStorage('shipping_address_phone');  
  removeStorage('coleta_address_phone');  
  removeStorage('precoleta_address_phone');  
  removeStorage('retorno_address');  
  removeStorage('merchant_id');
  removeStorage('data_da_entrega');
  removeStorage('hora_da_entrega');
  removeStorage('transaction_type');
...
{	
   ...
   var options = {
      ...
      onTransitionEnd: function() { 		
		  ...
		  if(data.msg.show_mobile_number == 'true' || data.msg.show_mobile_number == true){
			$(".contact_phone").val($(".contact_phone").masked( data.msg.profile.contact_phone.replace("+55","") ));  
		  } else {
			  $(".contact_phone_titulo").hide;
			  $(".telefone_contato_titulo").show;
...
function showShippingLocationColeta(data)
{	
   var options = {
      animation: 'slide',
      onTransitionEnd: function() { 		
...
function showShippingLocationPreColeta(data)
{	
...
function cancelCartOrderEntregadores()
{
  if ( cart.length>0 ){		
	   ons.notification.confirm({
		  message: getTrans('Cancel current order?','cancel_order') ,	  
		  ...
		  callback: function(index) {	  	       
		  	   if (index==0){	  	   	      	   	  
					sNavigator.popPage({cancelIfRunning: true}); 
				   menu.setMainPage('searchResults.html', {closeMenu: true});
		  	   }
		  }
		});
  } else {
...
displayOptionsList = function(data){
	...
	$.each( data.list, function( key, val ) {
		htm+='<ons-list-item modifier="tappable" onclick="setOptionValue(' + "'" + key + "'," + "'" + val + "'"  + ');" >';
		   htm+= '<span class="small-font-dim-verde">'+val+'</span>';
		htm+='</ons-list-item>';
	});
	htm+='</ons-list>';	
	createElement('options_list',htm);	
...
setOptionValue = function(key, value){
	...
	switch(option_type){
		case "DateList":
		  ...
		  $(".data_da_entrega").val(key);
		  setStorage("data_da_entrega",key);
		  $(".cart_delivery_date_value").html(value);
		break;
		
		case "TimeList":
		  $(".delivery_time").val(key); 
		  $(".hora_da_entrega").val(key); 
		  setStorage("hora_da_entrega",key);
